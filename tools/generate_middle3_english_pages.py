...
PARENT = "과목별학원"
CATEGORY = "중3영어학원"
TITLE_SUFFIX = " 중3 영어학원"
SUBJECT_LABEL = "중3 영어"
SUBJECT = "영어"
SUBJECT_EN = "ENGLISH"
GRADE_TEXT = "중학교 3학년"
FOCUS_LABEL = "내신·고1 연계 대비"
ZIP_PATH = BASE / "참고자료" / "사용한 원고" / "영수학원.com 추가 원고" / "중3 영어학원.zip"

# Categories in the 과목별학원 tree — 존재하지 않으면 건너뛰므로 미리 전부 등록해 둔다.
SIBLING_CATEGORY_META: dict[str, str] = {
    "중2수학학원": "중2 수학 내신·오답관리 지역별 안내",
    "중2영어학원": "중2 영어 내신·서술형 대비 지역별 안내",
    "중3수학학원": "중3 수학 내신전략·오답관리 지역별 안내",
    "중3영어학원": f"중3 영어 {FOCUS_LABEL} 지역별 안내",
}
...
# 이 원고 배치는 FAQ가 "Q1. 질문\n답변(마커 없음)" 형태이고(A. 마커 자체가 없음),
# 학부모후기도 빈 줄 구분 없이 "note\nquote1\nquote2"가 줄바꿈만으로 붙어 있다.
# ---------------------------------------------------------------------------
...
def parse_review(value: str) -> tuple[str, list[str]]:
    """이 원고는 note/quote가 빈 줄 없이 한 줄씩 이어져 있어 줄 단위로 분리한다."""
    ...
    for line in lines:
        if line.startswith("※"):
            ...
            quotes.append(line)
    if not quotes:
        raise ValueError("학부모후기를 해석하지 못했습니다.")
    return note, quotes
...
def contextualize_repeated_paragraph(
    ...
    *,
    ...
    evidence = [
        "최근 시험지의 서술형 채점 기준", "학교 교과서 본문과 문법 포인트", "주간 단어 시험 결과",
        "해설 없이 다시 쓴 영작", "지문별 오답 유형", "수행평가 준비 기록",
        "수업 뒤에 남긴 질문 목록", "시험 전 남은 학습일", "교재별 완료 범위",
        "오답을 다시 확인할 날짜", "학생이 설명한 해석 근거",
    ][digest[0] % 11]
    outcome = [
        "다음 점검 시점", "우선 복습 단원", "혼자 다시 풀 지문", "질문 순서",
        "시험 전 완료 범위", "과제량 조절 시점", "보충 설명이 필요한 문법 포인트",
...
CHECKLIST_BANK: list[tuple[str, str]] = [
    ("최근 영어 시험지", "점수보다 어휘·문법·독해·서술형 중 어디서 틀렸는지 확인하는 데 필요합니다."),
    ("학교 시험 범위", "다니는 학교의 시험 범위와 수행평가 일정을 확인합니다."),
    ("현재 교재·단어장", "지금까지 사용한 교재와 단어 암기 범위를 확인해 시작 지점을 잡습니다."),
    ("오답·서술형 정리 습관", "기존에 오답과 서술형 감점을 정리해 온 방법이 있다면 함께 확인합니다."),
]

COMPARE_ROWS: list[dict[str, str]] = [
    {"label": "학습 진단", "other": "정해진 진도만 그대로 진행", "ours": "학교 시험 범위부터 먼저 확인"},
    {"label": "내신·고1 연계", "other": "당장 문제 수만 늘림", "ours": "고1 연계까지 고려한 누락 보완"},
    {"label": "서술형 관리", "other": "채점만 하고 다음 진도로 이동", "ours": "감점 원인을 구분한 뒤 재작성 확인"},
...
def page_ld(
    *,
    ...
    about = [
        ...
        {"@type": "Thing", "name": f"{GRADE_TEXT} {SUBJECT}"},
        {"@type": "Thing", "name": "내신·고1 연계 대비"},
        {"@type": "Thing", "name": "서술형 관리"},
    ]
    mentions = [
    ...
    has_part = section_names + ["센터 기준 정보", "학습료 안내", "상담 전 체크리스트", "FAQ", "학부모 상담 후기", "근처 학원페이지"]
    ...
    return {
        ...
        "@graph": [
            ...
            },
            {
                ...
                "knowsAbout": [SUBJECT_LABEL, "내신 대비", "고1 연계 준비", "서술형 관리", "학습 상담"],
                "makesOffer": [
                    {"@type": "Offer", "itemOffered": {"@type": "Service", "name": f"{local} {SUBJECT_LABEL} 진단 상담", "serviceType": "TutoringService"}},
                    {"@type": "Offer", "itemOffered": {"@type": "Service", "name": f"{local} 내신·고1 연계 대비 관리", "serviceType": "TutoringService"}},
                    {"@type": "Offer", "itemOffered": {"@type": "Service", "name": f"{local} 서술형 재학습 관리", "serviceType": "TutoringService"}},
                ],
            },
            {
                "@type": "Article",
...
def detail_page(
    ...
    fit_section = f"""    <section class="section">
      ...
      <div class="card-grid">
        ...
        <article class="info-card"><span class="tag">학년</span><h3>{esc(GRADE_TEXT)}</h3><p>{esc(local)} 학생은 같은 학년이어도 어휘·문법·독해·서술형의 약점과 고등 준비 시점이 다르므로 최근 자료를 진단한 뒤 우선순위를 정합니다.</p></article>
        <article class="info-card"><span class="tag">추천</span><h3>이런 학생에게 추천</h3><p>{esc(local)}에서 시험 직전 몰아치기보다 누적 복습을 시작하고, 고1 연계와 내신 서술형 대비의 순서를 함께 세우려는 학생에게 적합합니다.</p></article>
      </div>
    ...
    review_note_html = f'<p class="lead">{esc(review_note)}</p>' if review_note else ""
    review_section = f"""    <section class="section">
      <div class="section-head">
        <p class="eyebrow">PARENT REVIEW</p>
        <h2>{esc(local)} {esc(SUBJECT_LABEL)}학원 상담 후기</h2>
        {review_note_html}
      </div>
  ...
  <main>
    <section class="page-hero">
      ...
      <p class="eyebrow">MIDDLE 3 {SUBJECT_EN} LOCAL GUIDE</p>
      <h1>{esc(title)}</h1>
...
def category_hub(rows: list[dict[str, str]]) -> None:
  ...
  <main>
    <section class="page-hero">
      ...
      <p class="eyebrow">MIDDLE 3 {SUBJECT_EN} DIRECTORY</p>
      <h1>{CATEGORY}</h1>
