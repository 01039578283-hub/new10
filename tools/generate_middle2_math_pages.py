...
def detail_page(
    ...
    fit_section = f"""    <section class="section">
      ...
      <div class="card-grid">
        ...
        <article class="info-card"><span class="tag">학년</span><h3>{esc(GRADE_TEXT)}</h3><p>{esc(local)} 학생은 같은 학년이어도 단원별 결손이 다르므로 최근 풀이를 진단한 뒤 우선순위를 정합니다.</p></article>
        <article class="info-card"><span class="tag">추천</span><h3>이런 학생에게 추천</h3><p>{esc(local)}에서 학원 변경을 고민하거나, 학교 진도는 따라가지만 시간이 지나면 개념이 흐려지고 내신 대비의 순서를 잡기 어려운 학생에게 적합합니다.</p></article>
      </div>
